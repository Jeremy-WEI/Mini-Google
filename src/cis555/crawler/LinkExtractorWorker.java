...
public class LinkExtractorWorker extends Thread {
    ...
    private static final Logger logger = Logger
            .getLogger(LinkExtractorWorker.class);
    ...
    public LinkExtractorWorker(
            BlockingQueue<RawCrawledItem> contentForLinkExtractor,
            BlockingQueue<URL> preRedistributionNewURLQueue, int id,
            CrawlerDao dao, Set<String> sitesCrawledThisSession)
    ...
    /**
     * Cleans the document, adds meta data to database, and store file to
     * permanent storage (if it's a newly crawled file)
     */
    @Override
    public void run() {
        while (Crawler.active) {
            try {
                ...
                if (contentType == ContentType.HTML) {
                    if (null == cleansedDoc) {
                        String stringContents = new String(rawContents,
                                CrawlerConstants.CHARSET);
                ...
                logger.info(CLASSNAME + " Redistribution queue size: "
                        + this.preRedistributionNewURLQueue.size());

            } catch (UnsupportedEncodingException | IllegalStateException
                    | IllegalArgumentException e1) {
                logger.info(CLASSNAME + " Unable to decode, skipping "
                        + e1.getMessage());
            } catch (Exception e) {
        ...
        logger.info(CLASSNAME + ": LinkExtractorWorker " + this.id
                + " has shut down");
     ...
     * @throws NoSuchAlgorithmException
     */
    private void addAHrefLinks(Document cleansedDoc, boolean isNew,
            URL sourceUrl) throws IllegalArgumentException,
        ...
        for (Element link : links) {
            ...
            if (!urlString.isEmpty()) {
                if (!urlString.contains("mailto:")) { // We're ignoring urls
                                                      // with 'mailto's
                    URL newUrl = null;
                    try {
                        ...
                        if (null != newUrl) {
                            ...
                            if (!this.sitesCrawledThisSession
                                    .contains(newUrlString)) {
                                ...
                                this.dao.addNewDocumentMeta(newUrl.toString(),
                                        getDocID(newUrl), new Date(), false);
                        ...
                        logger.info(CLASSNAME + " :" + newUrl
                                + " syntax is invalid " + e.getMessage());
                    } catch (IllegalStateException e) {
                        logger.info(CLASSNAME
                                + " New preRedistributionNewURLQueue is full, dropping "
                                + newUrl);
            ...
            // (urlStringTime.getTime() - start.getTime()) +
            // "ms to extract link, " +
            // (filterString.getTime() - urlStringTime.getTime()) +
            // "ms to filter the url" +
            // (checkContained.getTime() - filterString.getTime()) +
            // "ms to check if we've added the link already " +
            // (putInQueue.getTime() - checkContained.getTime()) +
            // "ms to add url into the queue " +
            // (putInDatabase.getTime() - putInQueue.getTime()) +
            // "ms to put URL into the database");
        ...
        logger.info(CLASSNAME + "Extractor " + this.id + " took "
                + (savingToDatabase.getTime() - startExtractingLinks.getTime())
                + "ms to extract links "
                + (done.getTime() - savingToDatabase.getTime())
                + "ms to save to database");
    }
     ...
     * @throws MalformedURLException
     */
    private void addImgSrcLinks(Document cleasnedDoc, URL sourceUrl)
            throws IllegalArgumentException, UnsupportedEncodingException,
            MalformedURLException {
        Elements links = cleasnedDoc.select("img[src]");

        // List<String> addedImgLinksForOnePage = new ArrayList<String>();
        Set<String> addedImgLinksForOnePage = new HashSet<String>();
        ...
        for (Element link : links) {
            ...
            if (!urlString.isEmpty()) {
                if (!addedImgLinksForOnePage.contains(urlString)) {
                    URL newUrl = null;
                    try {
                        String cleansedString = URLDecoder.decode(urlString,
                                CrawlerConstants.CHARSET);
                        ...
                        logger.info(CLASSNAME + " Queue is full, dropping "
                                + newUrl);
     ...
     * @throws NoSuchAlgorithmException
     */
    private String getDocID(URL url) throws UnsupportedEncodingException,
            NoSuchAlgorithmException {
